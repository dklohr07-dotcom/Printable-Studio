...
const example = "Create a beginner herb garden planner. Include garden goals, herb wish list, herb profile sheet, planting planner, container planner, watering log, sunlight tracker, fertilizer tracker, harvest tracker, seed inventory, garden notes, and seasonal reflection pages. Calm, sophisticated botanical style.";
function esc(value) {
...
function toast(msg, error=false) {
 ...
 clearTimeout(toast.timer);
  // Keep actionable errors on screen instead of hiding their detail.
  if(!error)toast.timer=setTimeout(()=>el.style.display="none",7600);
}
...
async function exportMarketplace(kind){
  ...
  try {
    ...
    if(!response.ok){
      ...
      throw new Error(`Export ${response.status}: ${error.error||"Failed"}. You can still download the project backup below.`);
    }
...
function downloadProjectBackup(){
  if(!app.project){toast("Generate a collection before downloading a backup.",true);return;}
  const blob=new Blob([JSON.stringify(app.project,null,2)],{type:"application/json"});
  const url=URL.createObjectURL(blob),a=document.createElement("a");
  const name=app.project.title.toLowerCase().replace(/[^a-z0-9]+/g,"-").slice(0,56)||"printable";
  a.href=url;a.download=name+"-project.json";document.body.appendChild(a);a.click();a.remove();
  setTimeout(()=>URL.revokeObjectURL(url),60000);
  toast("Editable project backup prepared.");
}
...
async function init(){
 ...
 $("pdfBtn").addEventListener("click",()=>exportAsset("pdf"));
  $("topPdfBtn").addEventListener("click",()=>exportAsset("pdf"));
  $("projectBackupBtn").addEventListener("click",downloadProjectBackup);
 ...
 try{
  ...
  $("modeHint").textContent=app.config.aiConfigured?
   ...
   "To unlock AI Studio: add OPENAI_API_KEY in Railway → your service → Variables, then deploy the change. Demo and PDF downloads work without it.";
